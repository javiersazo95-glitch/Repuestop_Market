...
| A15 | **Imágenes en el chat de mediación** | `POST /conversaciones/{id}/mediacion-imagenes` | **IMPLEMENTADO (Fase 3)** | Backend: `MediacionChatController.java:34`<br>Mobile: `mobile/utils/conversations.ts:192`<br>Web: Implementado `uploadMediationChatImageApi` en `api.js` y botón de adjuntar foto en compositor directo de `MediationCaseView.jsx`. | `src/services/api.js`<br>`src/components/MediationCaseView.jsx` |
| A16 | **Búsqueda por catálogo de vehículo** | `/catalogos/inventario/vehiculo-catalogos`, `/vehiculos-catalogo/{id}/repuestos` | **PENDIENTE** | Backend: `CatalogoInventarioController.java:244`, `RepuestoController.java:28`<br>Mobile: `mobile/utils/product-catalog.ts:757` (filtra catálogo, tienda y directorio)<br>Web: las funciones existen en `api.js` pero **ninguna vista las usa**. `activeVehicle` es decorativo: muestra "Compatible con {patente}" y su `catalogoId` se adapta sin consumirse. Conectarlo exige una segunda fuente de datos en `PartsCatalogView` (`RepuestoOfertaPageDTO` con ofertas anidadas ≠ listado plano). | `src/pages/CatalogPage.jsx`<br>`src/components/PartsCatalogView.jsx` |
